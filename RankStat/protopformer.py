...
class PPNet_Normal(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
        ...
        if add_on_layers_type == 'bottleneck':
            ...
            #     )
            self.add_on_layers = nn.Sequential(
                nn.Linear(first_add_on_layer_in_channels, 1024),
                nn.GELU(),
                nn.Linear(1024, 256),
                nn.GELU(),
                nn.Linear(256, self.prototype_shape[1]),
                nn.GELU()
            )
        ...
        self.hash_head = HASHHead(in_dim=self.prototype_shape[1], code_dim=12)
    ...
    def forward(self, x):
        ...
        if not self.training:
            ...
            return logits_global, (_, _, _, logits_global), hash_feat, cls_tokens
